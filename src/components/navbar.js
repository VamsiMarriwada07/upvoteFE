    ...
    <div className="main flex flex-row items-center justify-around">
      ...
      <div className="options flex flex-row justify-end mr-[-30px]">
        {openProfile && (
          <div className="flex flex-col dropdown">
            <ul className="flex flex-col gap-4 ">
              <li>
                <button onClick={handleDelete} className="mt-2 text-sm">
                  Logout
                </button>
              </li>
            </ul>
        ...
        {auth && (
          ...
          >
            <p className="text-md">
              Welcome, <span className="text-xl font-bold">{name}</span>
            </p>
          </div>
        )}
        {!auth && (
          <button
            className=" font-bold hover:text-pink-600 ease-out duration-40"
